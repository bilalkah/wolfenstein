...
Player::Player(const PlayerConfig &config)
    : pose(config.initial_pose_), player_speed_(config.player_speed_),
...
void
Player::Movement(const double delta_time)
{
  double dx = 0.0;
  double dy = 0.0;
...
void
Player::Update(const double delta_time)
{
  Movement(delta_time);
...
std::vector<Ray>
Player::GetRays() const
{
  return ray_;
}

void
Player::SetMap(const std::shared_ptr<Map> &map)
{
  map_ptr_ = map;
...
void
Player::MoveIfNotCollision(double dx, double dy)
{

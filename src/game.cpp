...
void Game::Init() {
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    ...
    if (window_ == nullptr) {
      DEBUG_MSG("Window could not be created! SDL_Error: " << SDL_GetError());
      isRunning_ = false;
    } else {
    ...
    for (uint16_t j = 0; j < row_size; j++) {
      if (map_data[j][i] != 0) {
        SDL_Rect rect = {config_.padding_ + j * config_.scale_,
                         config_.padding_ + i * config_.scale_, config_.scale_,
                         config_.scale_};
